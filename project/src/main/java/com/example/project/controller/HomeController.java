...
public class HomeController {
        ...
        try {
            ...
            if (listResp != null && !listResp.isEmpty()) {
                ...
                if (results != null && results.length() > 0) {
                    ...
                    bannerMap.put("backdrop",
                            IMAGE_BASE_URL + "/original" + (backdropPath != null ? backdropPath : ""));
                    bannerMap.put("rating", firstMovie.optDouble("vote_average", 0.0));
                    bannerMap.put("year",
                            (releaseDate != null && releaseDate.length() >= 4) ? releaseDate.substring(0, 4) : "2024");
                    ...
                    if (movieId > 0) {
                        try {
                            ...
                            if (detailResp != null && !detailResp.isEmpty()) {
                                ...
                                if (runtime > 0)
                                    bannerMap.put("runtime", runtime);
                                else
                                    bannerMap.put("runtime", 0); // fallback if not provided
    ...
    private List<Map<String, String>> getMoviesFromUrl(RestTemplate restTemplate, String url, int startIndex,
            int maxCount) {

...
// // trong header fragment hoặc search.js
// (function(){
//   const bc = ('BroadcastChannel' in window) ? new BroadcastChannel('movie-filters') : null;
//   const input = document.getElementById('searchInput');
//   if(!input) return;
//   input.addEventListener('input', (e) => {
//     if(bc) bc.postMessage({ q: e.target.value });
//   });
// })();

// async function loadGenres(){
//   try {
//     const res = await fetch('/api/genres', { headers: { 'Accept':'application/json' }});
//     if(!res.ok) return;
//     const json = await res.json(); // expect [{id,name},...]
//     const sel = document.getElementById('genres-select') || document.getElementById('genres-select-fallback');
//     if(!sel) return;
//     sel.innerHTML = '<option value="">Tất cả thể loại</option>' + json.map(g => `<option value="${g.id}">${g.name}</option>`).join('');
//   } catch(e){ console.warn('loadGenres error', e); }
// }
// document.addEventListener('DOMContentLoaded', loadGenres);

// document.getElementById('main-nav')?.addEventListener('change', (e) => {
//   const theme = e.target.value;
//   if(window.BroadcastChannel) new BroadcastChannel('movie-filters').postMessage({ theme });
//   // if on index page and you have rows already, filter client-side:
//   const cards = document.querySelectorAll('.movie-card');
//   cards.forEach(c => {
//     const tags = c.dataset.quick || c.dataset.tags || '';
//     // apply logic based on theme (you may set data-theme on server for each card)
//     if(theme === '' || c.dataset.theme === theme) c.style.display = '';
